...
{
	if ( _index < 0 || _index > str_size )	// Should have used exeption fault
	{
...
{
	if ( _index < 0 || _index > str_size ) // TODO exeption fault
	{
		std::cout << "Index error!!!" << std::endl;
		return;
	}
...
{
	if ( _pos < 0 || _pos > str_size || _len < 0 
		|| _len > str_size || _pos + _len > str_size ) 
	{
		std::cout << "Index error!!!" << std::endl;
		return;
	}
...
void String::erase( It<char> _first, It<char> _last ) 
{
	...
	for ( size_t it = _f; str_data[it] != *_last; it++ )
		_l++;
	if (_f > _l) {
		std::cout << "Index error!!!" << std::endl;	// TODO exeption fault
		return;
	}
	this->erase(_f, _l - _f);
}
